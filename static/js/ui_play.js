...
var playObj = {
	...
	init : function(opts){
		...
		if(isTest)
		{
			that.parseUrl(vod_test.data[this.data.id].url);
			GF_WebPrint(opts.url);
			that.playFromStart();
		}
		else
		{
	...
	back : function()
	{
		...
		{
			GF_WebPrint("1111111111111111111111");
			this.finalize();
			GF_WebPrint("22222222222222222222222");
			//返回到上一页
			//var backstr = $.cookie('letv_info');
			GF_WebPrint("333333333333333333");
			//var tmparr = backstr.split('|');
			GF_WebPrint("4444444444444444444");
			//增加 page 参数
	...
	hidePage : function()
	{
		...
		$('#nav').show();
		$('#recommend').show();
		GF_WebPrint("55555555555555555");
		hideTimeBox();
		//$('#div_box').show();
		//document.body.style.opacity = 1;	//透明化
		//同步光标
		GF_WebPrint("66666666666666666666666");
		infoObj.init_tv(this.getIndex());
		GF_WebPrint("77777777777777777");
		infoObj.event();
		GF_WebPrint("88888888888888888888");
	},
	select : function()
	{
		if(this.data.mouseFlag=='swap'){
	...
	moveBox : function(selectIndex)
	{
		...
		o("div_box2").style.left = left + 'px';
		o("div_box2").style.top = top + 'px';
	...
	loadNextTV : function()
	{
		...
		//暂时注销 window.location.href = 'play_video.html?page='+ this.data.page +'&id=' + this.data.id + '&tvid=' + sid + '&index=' + index + '&name=' + encodeURI(that.data.title);
		that.init({
			id : this.data.id,
			mmsid : sid,
	...
	parseUrl : function(tmpUrlStr)
	{
		...
		if(isTest)
		{
			var tempUrls = "高清|"+tmpUrlStr+"#"+"标清|"+tmpUrlStr+"#"+"流畅|"+tmpUrlStr;
			tmpUrlStr = tempUrls;
		}
	...
	sysEvent : function()
	{
		...
		GF_WebPrint("111111111111111111111111111"+tmptype);
		...
		else if(tmptype == 'EVENT_PLAYMODE_CHANGE')
		{
			...
			{
				//播放结束 跳转
				GF_WebPrint("1111111111111111111");
				clearInterval(cronStatus);
				GF_WebPrint("222222222222222");
				this.showStatusDiv();
				GF_WebPrint("33333333333333333333333");
				$(".prognameTip").html('播放失败');
				GF_WebPrint("4444444444444444");
			}
			else if(tmpstatus==4)
			{
				//播放结束 跳转
