...
function getProgdata(retried){
	...
	proList = [];
	try{
		var url = MY_PORTAL_ADDR+"/GetSearch?name="+name+"&startAt=1&maxItems=1000";
		Ajax.request(url,{
...
var buttonArea = (function(){
	...
	area.myfocus = function(){
		if(area.index == 0){
			...
			document.getElementById("search").style.background="url(images/search1.png) no-repeat";
		}
	}
	area.myblur = function(){
		if(area.index == 0){
			...
			document.getElementById("search").style.background="url(images/search.png) no-repeat";
		}
	}
	area.href = function(){
		if(area.index == 0){
