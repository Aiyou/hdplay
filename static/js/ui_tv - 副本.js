...
function showMenuInTimer()
{
	if(menuStatus != null)
	{
		clearTimeout(menuStatus);
	}
	menuStatus = setTimeout(hideMenuInTimer,6000);
}
function hideMenuInTimer()
{
	menuStatus = null;
	tvObj.data.mouseFlag = 'play';
	hideDiv('ch_bg');
	hideDiv('ch_detail');
	hideDiv('div_msg');
	hideDiv('up_arrow');
...
var tvObj = {
	...
	initDate : function()
	{
		...
		{
			this.date = new Date();
			this.date.setTime(this.data.timeStamp);
	...
	initVars : function(toPlay)
	{
	  ...
	  if(!toPlay) return;
		...
		if(last_play_str)
		{
			...
			//window.alert(menuIndex+','+tvIndex);
		...
		//尽快显示节目信息面板
		this.showProgramInfo(this.data.programID,this.data.programName + ' Connecting...');
		showDiv('content');
		//window.alert(last_play_str);
		...
		getPlayLink(menuContent[0][0].id);
		//$("#ch_lists li.curfocus").removeClass('curfocus');
	},
	...
	moveBox : function(selectIndex)
	{
		...
		//$("#div_msg").css('top',idx + 'px');		//定义光标框的顶部坐标
	...
	resetBox : function()
	{
		//此函数用于 分类列表上下切换时把节目列表的状态显示成从第一个节目开始 防止按右键后 看到的菜单不是刚刚看到的
		//$("#div_list_" + menuIndex).css('top','0px');
	},
	...
	playNextTV : function()
	{
		document.getElementById("debug").innerText = "playNextTV 1111111111111111111";
		//if(this.data.programID >= this.data.totalTV)
		//{
		//	return false;
		//}
		document.getElementById("debug").innerText = "playNextTV 22222222222222222222";
		//只要可以切换 立即停止节目 
		this.stop();
		//赶紧响应显示面板
		if(this.data.classPage != CurrentPageIndex)
		{
			document.getElementById("debug").innerText = "playNextTV 3333 thispage="+this.data.classPage+", currentPageIndex="+CurrentPageIndex;
			jumpToPage(this.data.classPage);
		}
		menuIndex = this.data.index;
		tvIndex = this.data.tvIndex;
		if(menuIndex == menuArr.length-1 && tvIndex == menuContent[menuIndex].length-1)
		{
			document.getElementById("debug").innerText = "playNextTV 444444444444444 thispage="+this.data.classPage+", currentPageIndex="+CurrentPageIndex;
				classPageDown();
				this.data.classPage = CurrentPageIndex;
				menuIndex = 0;
				tvIndex = 0;
		}
		else
		{
			document.getElementById("debug").innerText = "playNextTV 55555555555555555 thispage="+this.data.classPage+", currentPageIndex="+CurrentPageIndex;
		...
		//document.getElementById("debug").innerText = "playNextTV 6666666666666 thispage="+this.classPage+", currentPageIndex="+CurrentPageIndex;
		var program_id = menuContent[menuIndex][tvIndex].chid;
		this.data.programID = program_id;
		this.data.programName = menuContent[menuIndex][tvIndex].name;
		this.showProgramInfo(program_id,this.data.programName + ' Connecting...');
		showDiv('content');
	...
	playPrevTV : function()
	{
		...
		if(menuIndex == 0 && tvIndex == 0)
		{
			classPageUp();
			this.data.classPage = CurrentPageIndex;
			menuIndex = menuArr.length-1;
			tvIndex = menuContent[menuIndex].length-1
		}
		else
		{
			if(tvIndex>0)
		...
		this.data.programID = program_id;
		this.data.programName = menuContent[menuIndex][tvIndex].name;
		this.showProgramInfo(program_id,this.data.programName + ' 切换中,请稍等');
		showDiv('content');
	...
	down : function()
	{
		if(this.data.mouseFlag == 'list')
		{
			...
			if(tvIndex >= this.data.indexArr[menuIndex])
			{
				...
				var htmlstr = '';
				var count = 9;
				if(this.data.indexArr[menuIndex] < 9)
					count = this.data.indexArr[menuIndex] ;
	...
	right : function()
	{
		if(this.data.mouseFlag == 'class')
		{
			...
			//this.data.listObjs[startIndex].setAttribute('class','curfocus');
			tvIndex = startIndex;
			this.showBox();
	...
	showTimeInfo : function()
	{
		...
		/*显示节目信息
		var tmparr = this.getEpgInfo(1,1);
	...
	playTV : function()
	{
		getPlayLink2(menuContent[this.data.index][this.data.tvIndex].id);
		//var mediaStr = this.toJson(this.data.tvUrl);
		//this.mp.setSingleMedia(mediaStr);
		//this.mp.playFromStart();
	},
  playTVFilmOn:function(url)
  {
  	document.getElementById("debug").innerText = url;
  	this.data.tvUrl = url;
  	var mediaStr = this.toJson(this.data.tvUrl);
	...
	sysEvent : function()
	{
		...
		if(tmptype == 'EVENT_PLAYMODE_CHANGE')
		{
			...
			if(tmpstatus == 4)
			{
				...
				showDiv('content');
				showLoading();
				o("spanChannelName").innerHTML = this.data.programName + ' Buffering...';
			}
			else if(tmpstatus ==6)
			{
				clearTimeout(cronStatus);
				showDiv('content');
				hideLoading();
				o("spanChannelName").innerHTML = this.data.programName + ' End';
			}
			else if(tmpstatus ==5)
			{
				clearTimeout(cronStatus);
				showDiv('content');
				hideLoading();
				o("spanChannelName").innerHTML = this.data.programName + ' Failed! Please change channel';
			}
			else if(tmpstatus == 2)
			{
				//3秒之后隐藏信息面板
	...
	event : function()
	{
		var that = this;
		 document.onkeypress = function(e){
			...
			case 48:
				//exitSwapBox();
				window.location.href = 'list2.php';
				break;
