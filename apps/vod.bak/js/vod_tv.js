// JavaScript Document
var request_child_column_tv =  getCookie("request_child_column_tv");
if( ( request_child_column_tv == "") ||( request_child_column_tv == "undefined" ) ){
	request_child_column_tv = 0;//0:表示父级栏目,1:表示子级栏目
}

var area_Index =  getCookie("Tv_List_Area_Index");
...
var channelIndex =  getCookie("tv_list_Index");
if( ( channelIndex == "") ||( channelIndex == "undefined" ) ){
	channelIndex = 0;
}else{
	if( request_child_column_tv == 1 ){
		channelIndex = 0;
	}else{
...
var path = getCookie("TV_Path");//VOD栏目路径;
var path_name = unescape(getCookie("TV_Path_Name"));//VOD栏目路径名
...
var contentArea = function(){
	...
	o.handleChannelListsData = function (folderId){
		...
		success:function(data){
			...
			if( request_child_column_tv == 0 ){
			  addCookie("tv_list_Index", o.index);	
			}
		},
	...
	o.right = function(){
		if( epgArea.epgLists.length != 0 ){
			...
			epgArea.getfocus();
			if( request_child_column_tv == 0 ){
				addCookie("Tv_List_Area_Index", 1);
			}
		}
	}
	o.left = function(){
...
var epgIndex =  getCookie("Program_Index_Tv");
...
var epgArea = function(){
			...
			success:function(data){
				if(data.result == "200"){
					...
					if( o.epgLists.length != 0 ){
						if( o.epgLists.length - 1  < epgIndex ){
							o.index = 0;
						}
						o.showData();
						o.showPageArea();
	...
	o.getfocus = function(){
			...
			if( request_child_column_tv == 0 ){
	 			addCookie("Program_Index_Tv", o.index);	
			}
	}
	o.showData = function(){
		   $("epg_lists").innerHTML = "";
	...
	o.left = function(){	
	  if ( o.epgLists.length == 0 ) return ;
		...
		{  o.getblur();
			...
			contentArea.onFocus();
			if( request_child_column_tv == 0 ){
		   		addCookie("Tv_List_Area_Index", 0);	
			}
			/*o.index=o.channelLists.length-1;
			if(o.channelLists.length <= o.n){
				o.getfocus(o.index);
...
/*			o.getblur();
	...
	o.selects = function(){
		if( o.epgLists.length != 0 ){
			//addCookie("request_child_column_tv", 0);	
	...
	switch(val){
		...
		case ROC_IRKEY_EXIT:
			...
			if( ( pathArr.length == 1 )&&(pathArr[0] == "")){				 
				addCookie("request_child_column_tv", 0);	
				addCookie("TV_Path",getPathStr(path));
				addCookie("TV_Path_Name",getPathStr(path_name));
				window.location.href="../index/index.html";
			}else{
