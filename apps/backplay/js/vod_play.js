...
//		$("jt5").textContent = time.charAt(7);
	...
	function setJumpTime(type){
		var jumppace = Math.floor(parameter.progtime/40);	
		...
		try{
			...
			if( parameter.subId == "" ){
				...
				if( parameter.subId < tempUrl.length ){
					...
					GF_WebPrint(nextName+"-"+tempUrl[ parameter.subId-1 ]+"-"+ parameter.subId );
					setPlay( nextName, tempUrl[ parameter.subId - 1 ] , parameter.subId );
				}else{
					//smallwin.show("播放到尾是否退出");
					showSysError("播放结束");	
					setTimeout(function(){
...
var subListObj = function(){	
	...
	o.onSelect = function(){
		try{
			...
			GF_WebPrint(Name+"-"+curPlayUrl[ parameter.subId-1 ]+"-"+ parameter.subId );
			setPlay( Name, curPlayUrl[ parameter.subId - 1 ] , parameter.subId );	
		}catch(e){
	...
	function getData( retried ){
		try{
			//var url =MY_PORTAL_ADDR+"/GetItemData?id=" + parameter.programId;
			var url ="http://84.200.224.100"+"/GetItemData?id=" + parameter.programId;
			...
			Ajax.request(url,{
				success:function(data){
					...
					var programInfo = data.ItemsContents[0];
					...
					programCount = curPlayUrl.length;
