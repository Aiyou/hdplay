...
var navHandle = function(){
	...
	o.setNavObj = function(){
		...
		areaAll = $("areaList").getElementsByTagName("li");
		categoryAll = $("categoryList").getElementsByTagName("li");
		yearAll = $("yearList").getElementsByTagName("li");

		if( !isNaN(cookieObj.findCookie( "menuIndex" ) ) ){
			this.index = cookieObj.findCookie( "menuIndex" );
			areaIndex = cookieObj.findCookie( "areaIndex" );
			categoryIndex = cookieObj.findCookie( "categoryIndex" );
			yearIndex = cookieObj.findCookie( "yearIndex" );
			
			area = areaAll[areaIndex].innerHTML;
			category = categoryAll[categoryIndex].innerHTML;
			year = yearAll[yearIndex].innerHTML;
...
//	}
	...
	function setIndex(obj){
		switch(obj){
			...
			break;
			case 'category':
				categoryIndex = o.index - areaLength;
			break;
			case 'year':
				yearIndex = o.index - categoryLength - areaLength;
			break;			
		}
		menuFocus();
	}
	...
	function loseCurMenu(){
		...
		areaAll[areaIndex].style.color = "";
		categoryAll[categoryIndex].style.color = "";
		yearAll[yearIndex].style.color = "";

	}
	
	function menuFocus(){
		areaAll[areaIndex].style.color = "rgb(0, 51, 255)";
		categoryAll[categoryIndex].style.color = "rgb(0, 51, 255)";
		yearAll[yearIndex].style.color = "rgb(0, 51, 255)";
	}
	
	function searchFocus(){
		searchFlag = true;
		$("search").style.backgroundColor = "#06F";
	}
	...
	o.getLiIndex = function(){
		var tempData = {"area":areaIndex,"category":categoryIndex,"year":yearIndex};
		return tempData;
	}
...
//			pageButton = true;
	...
	function getData(id,page,func,retried,para){
		try{
			...
			var url = MY_PORTAL_ADDR+'/GetVodContents?id='+ id +'&startAt='+ page +'&maxItems='+row*col+'&area='+area+'&category='+category+'&year='+year;			
			//alert(url);
...
//		}
		if(pageCount == 0){
			...
			var sliderLen = (pageNo/pageCount)*582;
			$("slider").style.height = sliderLen +"px";
		}
	}
...
var cookieObj = function(){
	var o = {};
	var cookieValue = {"vod_menuid":"","menuIndex":"","pageNo":"","vod_movieIndex":"","areaIndex":"","categoryIndex":"","yearIndex":""};
	...
	function setCookie(){
		...
		addCookie("areaIndex",navHandle.getLiIndex().area);	
		addCookie("categoryIndex",navHandle.getLiIndex().category);		
		addCookie("yearIndex",navHandle.getLiIndex().year);		
	...
	function getAllCookie(){
		...
		cookieValue.areaIndex = getCookie("areaIndex");
		cookieValue.categoryIndex = getCookie("categoryIndex");
		cookieValue.yearIndex = getCookie("yearIndex");
	...
	function delCookie(){
		...
		deleteCookie("areaIndex");
		deleteCookie("categoryIndex");
		deleteCookie("yearIndex");
		deleteCookie("pageNo");
